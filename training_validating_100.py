...
# read the csv file
filename = "result.csv"
df = pd.read_csv('./results/{}'.format(filename))
print(filename,"->  Shape:",df.shape)

## CONCATENATE THE VECTORS THAT CARACTERIZE THE IMAGES
df['phi'] = df['phi'].apply(ast.literal_eval)
df['haralick'] = df['haralick'].apply(ast.literal_eval)
df['lbp'] = df['lbp'].apply(ast.literal_eval)
...
classifier_3NN = KNeighborsClassifier(n_neighbors=5, metric='minkowski')
