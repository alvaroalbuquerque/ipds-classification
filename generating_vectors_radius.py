...
for count, line in enumerate(open(class_file, 'r').readlines()):
    ...
    for radius_curr in range(2, 3):
        # get image vectors
        phi,haralick,lbp = getVvector(image, radius_curr, t0)

        # append result
        results.append((radius_curr, name, phi.tolist(), haralick.tolist(), lbp.tolist(), classification))


# store result as csv file
df = pd.DataFrame(results, columns=['radius', 'name', 'phi', 'haralick', 'lbp', 'classification'])
...
df.to_csv('./results/result_radius.csv',index=False)
